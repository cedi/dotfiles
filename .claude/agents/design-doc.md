...
tools: Read, Write, Edit, Bash, Grep, Glob, WebFetch, WebSearch, Skill, TaskCreate, TaskUpdate, TaskList, TaskGet
---
...
Follow the phases in order. Use the TaskCreate tool to track which phase you are in
...
- **fogg-behavior-mode** -- When the doc involves adoption or behavior change
- **tit-for-tat** -- When there are multi-party dynamics or reciprocal structures
...
- **five-why** -- When the root cause needs deeper excavation
...
When creating a new document, first check if the project directory exists (use
Bash with `ls` or a glob). If not, create it.
